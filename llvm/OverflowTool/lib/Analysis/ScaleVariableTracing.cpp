    ...
    for (Value *V : scale_variables) {
        ...
        if (isa<AllocaInst>(V)) {
            ...
            if (GlobalVariable *gv = dyn_cast<GlobalVariable>(
                ...
                findGEPs(gv, geps);
            } else if (auto *allocaV = dyn_cast<AllocaInst>(
                           gep->getPointerOperand()->stripPointerCasts())) {
                errs() << "    it points to an alloca with type"
                ...
                if (auto *arrayV =
                    ...
                    errs() << "    And it is an arraytype with size "
    ...
    if (CallInst *callInst = dyn_cast<CallInst>(
        ...
        for (scale_node *parent : sg->getvertex(V)->parents) {
            ...
            for (unsigned int i = 0; i < callInst->getNumOperands(); ++i) {
                // errs() << "checking " << i << "th operand which is " <<
                ...
                if (callInst->getOperand(i) == parent->value) {
                    ...
                    if (fp->isDeclaration()) {
                        // errs() << "     Function body not available for
                        // further tracing. ( " << fp->getName()  << " )\n";
                    } else if (fp->isVarArg()) {
                        errs() << "     Function is variadic. Don't know how "
                                  "to trace: "
                               << fp->getName() << "\n";
                    } else {
                        // errs() << "     Tracing in function body of called
                        // function via " << i << "th argument. ( " <<
...
bool ScaleVariableTracing::gepsAreEqual(GEPOperator *a, GEPOperator *b) {
    ...
    if (areEqual) {
        // iterate over both sets if indices simultaneously (they are same
        // length)
...
*/
void ScaleVariableTracing::printTraces(
    Value *start, int depth, std::unordered_set<scale_node *> &visited,
    scale_graph *sg) {
    printTraces(sg->getvertex(start), depth, visited);
}

void ScaleVariableTracing::printTraces(
    scale_node *node, int depth, std::unordered_set<scale_node *> &visited) {
