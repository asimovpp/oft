...
/*
Find scale variables that are initiated as a mpi_comm_rank or mpi_comm_size
variable.
*/
std::vector<Value *>
LibraryScaleVariableDetection::findMPIScaleVariables(Function *func) {
    // List of scale functions in MPI. Names as found in C and Fortran.
    const std::unordered_set<std::string> mpi_scale_functions = {
        "MPI_Comm_size",      "MPI_Comm_rank",       "MPI_Group_size",
        "MPI_Group_rank",     "mpi_comm_size_",      "mpi_comm_rank_",
        "mpi_group_size_",    "mpi_group_rank_",     "mpi_comm_size_f08_",
        "mpi_comm_rank_f08_", "mpi_group_size_f08_", "mpi_group_rank_f08_"};
    ...
    for (inst_iterator I = inst_begin(*func), e = inst_end(*func); I != e;
         ++I) {
        // TODO: might have to use CallSite wrapper instead to also catch
        // "function invokation"
        if (auto *callInst = dyn_cast<CallInst>(&*I)) {
            if (mpi_scale_functions.find(getFunctionName(callInst)) !=
...
LibraryScaleVariableDetection::perform(Module &M, ModuleAnalysisManager &AM) {
    ...
    for (Module::iterator func = M.begin(), e = M.end(); func != e; ++func) {
        ...
        if (functions_to_ignore.find(func->getName()) ==
            functions_to_ignore.end()) {
            std::vector<Value *> func_scale_vars =
                findMPIScaleVariables(&*func);
            scale_variables.insert(scale_variables.end(),
                                   func_scale_vars.begin(),
                                   func_scale_vars.end());
        }
    }

    errs() << "--------------------------------------------\n";
    errs() << "Library scale variables found:\n";
